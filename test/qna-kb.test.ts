...
describe('검색 품질 (회귀 고정)', () => {
  ...
  it('끝 형태소가 다른 유사어("금융상품권")는 완화 경로를 통과하지 못한다 (Codex 7차)', () => {
    // '금융+융상+상품' 3/4 이 질문에 있지만 끝 bigram '품권'이 없다 — 상품권 질의에
    // 금융상품 Q&A 를 공식 근거처럼 돌려주면 안 된다 (첫·끝 bigram 필수 조건)
    expect(searchQna('금융상품권')).toEqual([]);
  });
