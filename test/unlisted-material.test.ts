...
describe('비상장사 중요사항 — 주요주주 분기 기한 (§5의2④ 단서)', () => {
  ...
  it('4분기 중 변동은 익년 2월 말 기한 + 2027 데이터 없음 경고', () => {
    const d = unlistedMajorShareholderDeadline('20261101');
    // 2027-02-28은 일요일. 2027년 공휴일 데이터가 없어 3/1(실제로는 삼일절)이 영업일로 취급된다 —
    // 데이터 없음 경고가 반드시 동반되어야 한다. 2027 데이터 추가 시 이 기한은 20270302로 바뀌는 게 맞다.
    expect(d.deadline).toBe('20270301');
    expect(d.warnings.some((w) => w.includes('2027'))).toBe(true);
  });
});
