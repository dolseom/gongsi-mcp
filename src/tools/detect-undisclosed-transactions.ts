...
import { loadDocument, isDocumentCached } from './read-disclosure.js';
import {
  resolvePopulation,
  type Population,
...
import { ensureCorpIndex, fetchJurirNo } from '../resolver/corp-index.js';
import { CAP_100, formatWon } from '../rules/thresholds.js';
...
import {
  ...
  isSecuritiesReport,
  isCancellationReport,
  isTypeAmbiguousReport,
} from './detect/report-types.js';
import {
  parseFilingCounterparties,
  classifyAmbiguousSubject,
  expectedSubjectClass,
  toFilingRef,
  type AmbiguousSubjectClass,
  type FilingDocRead,
} from './detect/filing-doc.js';
import {
  buildThresholdMap,
  judgeOverThreshold,
  goodsQuarterlyLogic,
...
import {
  isForeignAffiliateColumn,
  foreignAffiliateReason,
  looksLikeNaturalPerson,
  isQuarterlyGoodsTable,
  itemLikelyNotGoodsService,
} from './detect/signal-rules.js';
import { closedBucket, collectActionItems } from './detect/action-items.js';
import {
  warmJurirJoins,
  MAX_WARM_LOOKUPS,
  WARM_SEARCH_LIMIT,
  WARM_SUPPRESS_DAYS,
  WARM_LOGIC_VERSION,
  type WarmingDiagnostics,
} from './detect/warming.js';
import type {
  DetectDeps,
  FilingRef,
  J001CheckFields,
...
// ── 외부(도구 등록·테스트)가 이 경로로 쓰는 심볼 — 분리한 모듈에서 그대로 다시 내보낸다 ──
export { TIME_BUDGET_MS } from './detect/budget.js';
export {
  isBorrowingReport,
  isLendingReport,
  isGoodsServicesReport,
...
export { buildThresholdMap, judgeOverThreshold } from './detect/threshold.js';
export { looksLikeNaturalPerson, itemLikelyNotGoodsService } from './detect/signal-rules.js';
export type { DetectDeps } from './detect/types.js';
