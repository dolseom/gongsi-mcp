...
server.registerTool(
  'detect_undisclosed_transactions',
  {
    ...
    description:
      ...
      '- 자금 차입은 **건별 차입일 근접 대조**입니다 (높은 신뢰도): 차입일 근방(−90~+30일)에 같은 유형 ' +
      '공시가 있으면 j001_filing_near_date, 검색창 안 어딘가에만 있으면 j001_filing_in_window_only ' +
      '(한도 의결 커버일 수도, **부분 공시 누락**일 수도 있음), 없으면 미공시 후보. 기준금액은 같은 ' +
      '문서의 재무현황 자본으로 계산하되 **근사치**이며, 거래금액 100억원 이상만 자본과 무관하게 확실합니다\n' +
      '- 상품·용역은 연간 합계뿐이라 **(판매회사, 거래상대방) 연간 합산** ≥ 4×기준금액인 경우만' +
      '(어느 분기 하나는 반드시 기준 이상) 신호로 쓰되, 이 유형의 공시의무는 **상대방이 총수일가 ' +
      '20% 이상 출자 계열사 등일 때만 성립**(법 §26①4호)하는데 지분을 확인할 수 없어 전부 ' +
      '**조건부 후보**(candidate_if_counterparty_qualified)입니다\n' +
      '- 개별 차입이 기준 미달이어도 같은 상대방 연간 합산이 기준 이상이면 "기준 미달"로 단정하지 ' +
      '않습니다 (§4③ 동일 거래상대방·동일 거래대상 기준)\n' +
      '- 유가증권은 총괄 **매트릭스 표**(매입회사×매도회사)의 상대방별 **연간 총액**만 알 수 있어 ' +
      '개별 거래로 분해되지 않습니다 — 총액이 기준 이상인데 유형 공시가 없으면 ' +
      'candidate_aggregate_only(미공시 후보가 아니라 **확인 대상**)로 냅니다. 반대로 총액이 기준 ' +
      '미만이면 그 상대방과의 개별 거래도 전부 미만이라 이 방향은 확실합니다\n' +
      '- 회사명→corp_code 조인 실패·검색 예산 초과·수집 불완전 건은 not_judged 로 분리됩니다 — ' +
      ...
      '있습니다. filing_near_date/in_window_only 도 "그 유형 공시가 존재한다"일 뿐 이 거래를 커버하는지는 ' +
      '대조하지 않았습니다. 응답의 **scope_caveats 를 반드시 사용자에게 함께 전달하세요.** ' +
